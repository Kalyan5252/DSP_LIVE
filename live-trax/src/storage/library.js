...
export function addFile(lib, { name, uri, bpm, analysis }, folderId = null, color = LIB_COLORS[0]) {
  const id = uid('fil');
  const files = {
    ...lib.files,
    [id]: { id, name: name || 'Loop', uri, color, tags: [], bpm: bpm > 0 ? bpm : null, analysis: analysis || null, folderId },
  };
